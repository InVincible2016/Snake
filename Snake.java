...
//Changlong Zhong's Snake game
//Built on May 26th, 2016
//Please refer to README for sources of images used.
//Have fun.
...
public class Snake extends JComponent implements ActionListener {
	...
	public static void main(String[] args) {
		...
		if (args.length != 0) {
			...
			s = new Snake(500, 2);// Default game mode at 30 fps and speed of 2;
		}
	...
	public Snake(int fps, int gamespeed) {
		try {
			...
			HeadImgUp = ImageIO.read(new File("SHeadU.png"));
			HeadImgDown = ImageIO.read(new File("SHeadD.png"));
			HeadImgLeft = ImageIO.read(new File("SHeadL.png"));
			HeadImgRight = ImageIO.read(new File("SHeadR.png"));
			BodyImg = ImageIO.read(new File("SnakeBody.png"));
			FoodImg = ImageIO.read(new File("Apple.png"));
		} catch (IOException e) {
	...
	public void drawMenuScreen(Graphics2D g) {
		g.drawImage(BackGroundImg, 0, 0, 800, 600, 0, 0, 400, 400, null);
		...
		g.setFont(TitleFont);
		g.setColor(Color.BLUE);
		g.drawString(Title, 180, 300);
		g.setFont(TitleFont2);
		g.drawString(My_Userid, 100, 360);
		g.setFont(InstructionFont);
		g.setColor(Color.RED);
		g.drawString(Instruction1, 490, 380);
		g.drawString(Instruction2, 530, 450);
	}
